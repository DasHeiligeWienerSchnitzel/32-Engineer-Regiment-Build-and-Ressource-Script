...
params ["_classname_list","_spawner","_spawnpoints","_ressources","_maxHeight","_minHeight"];

for "_i" from 0 to ((count _spawner) - 1) do {
	for "_j" from 0 to 3 do {
		(_spawner select _i) addAction
		[
			"Spawn Crate (" + (ER32_buildAndRessources_names select _j) + ")" , // Title
			{
				// Script
				params ["_target", "_caller", "_actionID", "_arguments"];
				_arguments params ["_spawnpoints","_i","_j","_ressources"];
				...
				if (count _nearbyCrates == 0) then {
					...
					private _crate = createVehicle [ER32_buildAndRessources_crates select _j, getPos (_spawnpoints select _i), [], 0, "CAN_COLLIDE"]; 
					...
					[_crate,ER32_buildAndRessources_names] remoteExecCall ["ER32_fnc_buildAndRessources_checkForRessources",0,true];
					[_crate] remoteExecCall ["ER32_fnc_buildAndRessources_loadOnFlatbed",0,true];
					
				}else{
					hint "Space occupied!";
				};
			},
			[_spawnpoints,_i,_j,_ressources,ER32_buildAndRessources_names],		// arguments
			1.5,		// priority
			true,		// showWindow
			true,		// hideOnUse
			"",			// shortcut
...
[_classname_list,_maxHeight,_minHeight] execVM "Scripts\ER32_buildAndRessources_addActions.sqf";