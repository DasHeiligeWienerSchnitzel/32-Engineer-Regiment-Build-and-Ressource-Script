...
/*
Gets nearest Flatbed that has the exact classname.
...
if (_object distance _nearestFlatbed < 15) then {
	...
	/*
	Now it will count how many objects are on the flatbed and depending on how many there are,
	...
	switch (count _objectsLoaded) do {
		case 0: {
			...
			*/
			...
			/*
			Only if the first object is laoded onto the flatbed the interaction to unload crates will be added onto the flatbed.
